...
router.post('/test-email', protect, async (req, res) => {
    try {
      ...
      res.status(500).json({ success: false, message: 'Error in test email route' });
    }
  });
...
router.post('/:bookingId/payment', protect, async (req, res) => {
    ...
    if (!numberOfPeople || !numberOfPeople.adults || numberOfPeople.adults < 1) {
      return res.status(400).json({
        success: false,
    ...
    const calculatedAmount = tourPackage.price.amount * 
    ...
    try {
      ...
      if (!emailSent) {
        console.warn(`Warning: Failed to send booking confirmation email to ${booking.contactDetails.email}`);
      }
    } catch (emailError) {
